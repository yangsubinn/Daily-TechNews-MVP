...
**Feed** — 전체 기사 목록을 최신순으로 확인

![Feed 뷰](assets/view-feed.png)

**날짜별** — 수집일 기준으로 날짜별 그룹핑, Tag별로 분류해서 확인
...
- 매일 아침 Notion을 열면 오늘의 테크 뉴스 브리핑이 준비되어 있음
- iOS, AI, LLM, Apple 등 관심 분야 키워드로 필터링된 뉴스만 수집
- 영문 기사를 한국어 제목 + 3줄 요약으로 제공
- Tags(multi-select)로 분야별 필터링 가능
...
매일 오전 7시 KST에 자동 실행된다.
GitHub 레포 → Settings → Secrets에 아래 3개 등록 필요:
...
Actions 탭 → Daily TechNews Brief → Run workflow로 수동 실행도 가능.
